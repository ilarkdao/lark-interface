<template>
  ...
  <!-- <b-button block variant="outline-primary" @click="emergencyWithdraw">紧急退出</b-button> -->
    <div class="delegate">
      <p class="titleSelected"> Vc Pool</p>
      <div class="delegatetext round-box">
          您的存量： {{ balanceOfDeposit }} VCT<br>
          VCT总存量： {{ balanceOfTotalLp }} VCT<br>
  ...
  export default {
    ...
    methods: {
      ...
      async getBalanceAndAllowance(){
				...
				let lpInstance = new this.web4.eth.Contract(abi, this.vcPoolToken)
      ...
      async letApprove(){
        ...
        this.isLoading = true
				let abi = this.$store.state.ercAbi
				let lpInstance = new this.web3.eth.Contract(abi, this.vcPoolToken)

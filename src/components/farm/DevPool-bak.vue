...
<!--    <div class="titleSelected notStart">创世挖矿尚未开启！</div>-->
<!-- <b-button block variant="outline-primary" @click="emergencyWithdraw">紧急退出</b-button> -->

    <!--DAI-LARK代理挖矿-->
    <div class="delegate">
      <p class="titleSelected"> Dev pool</p>
      <div class="delegatetext round-box">
          您的存量： {{ balanceOfDeposit }} DEV<br>
          DEV总存量： {{ balanceOfTotalDEV }} DEV<br>
  ...
  export default {
    ...
    methods: {
      ...
      checkDepositValue(){
        ...
        //代理量应小于SP量
        let res2 = parseFloat(this.depositValue) <= parseFloat(this.balanceOfDEV2)
        // console.log(699, "res2", res2)
        this.checkFlag = this.checkDelegateFlag = res && res1 && res2
        },
      async getBalanceAndAllowance(){
        let devInstance = this.$store.state.devtokenInstance2
        let addr = this.$store.state.addr
        let v = await devInstance.methods.balanceOf(addr).call()
        let DevBalanceOf = this.web4.utils.fromWei(v, "ether")
        this.balanceOfDEV= this.formatData(DevBalanceOf)
        this.balanceOfDEV2 = DevBalanceOf

        let t = await devInstance.methods.allowance(addr, this.larkPool).call()
      ...
      async letApprove(){
        ...
        this.isLoading = true
        let devInstance = this.$store.state.devtokenInstance
        ...
        await devInstance.methods.approve(this.larkPool, amount).send(option)
      ...
      async getDepositInfo(){
        ...
        let addr = this.$store.state.addr
        let f = await instance.methods.userInfo(2, addr).call()
        ...
        let devInstance = this.$store.state.devtokenInstance2
        let v = await devInstance.methods.balanceOf(this.larkPool).call()
        let DevBalanceOf = this.web4.utils.fromWei(v, "ether")
        this.balanceOfTotalDEV = this.formatData(DevBalanceOf)
      },

      fillMaxDeposit(){
        this.depositValue = this.balanceOfDEV
        if (parseFloat(this.depositValue) > 0){
      ...
      async deposit(){
        try {
          ...
          await instance.methods.deposit(2, amount).send(option)
      ...
      async harvest(){
        try {
          ...
          await instance.methods.harvest(2).send(option)
      ...
      async getPendingLark(){
        let instance = this.$store.state.poolInstance2
        let addr = this.$store.state.addr
        let res = await instance.methods.pendingLark(2, addr).call()
    ...
    mounted() {
      ...
      let that = this
      let instance = this.$store.state.devtokenInstance2
      console.log(236, Object.keys(instance).length)
      async function main(){
        if(Object.keys(instance).length === 0){
          try{
            await that.getPoolInstance()
            await that.getLarkInstance()
            await that.getDevInstance()
